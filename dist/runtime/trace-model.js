...
function countSteps(nodes) {
    ...
    for (const node of nodes)
        total += 1 + countSteps(node.children);
    return total;
}
...
function stepElapsedMs(step, nowMs) {
    ...
    if (step.status === 'running' && nowMs !== undefined)
        return Math.max(0, nowMs - step.startedAt);
    return undefined;
}
